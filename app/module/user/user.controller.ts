...
export const login = async (ctx: App.KoaContext) => {
    ...
    const password = crypto.createHmac('md5', sign)
                   ...
                   .digest('hex');
    const res = await UserModel.findOne({user: data.userName});
    if (!res || !res.id) {
...
export const createUser = async (ctx: App.KoaContext) => {
    ...
    const res = await UserModel.find({user: data.user});
    if (res.length !== 0) {
...
export const userInfo = async (ctx: App.KoaContext) => {
    ...
    const res = await UserModel.find({user: ctx.state.user.user}, {password: 0});
    if (res.length) {
        const userData = {
            name: res[0].user,
            userid: res[0]._id,
            currentAuthority: res[0].authority,
        }

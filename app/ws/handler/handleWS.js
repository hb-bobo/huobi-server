...
// const mysqlModel = require('../../models/table');
...
models.charts.watchSymbols.get().then(res => {
...
function handle(data) {
    ...
    switch (data.channel) {
        ...
        case 'kline':
            ...
            } else if(data.symbol === 'etcusdt') {
                appConfig.prices.eth = data.kline.close
            } else if(data.symbol === 'htusdt') {
            ...
            break;
        case 'trade':
            if (tradeHandles[data.symbol] === undefined) {
                tradeHandles[data.symbol] = new StatisticalTrade({
                    symbol: data.symbol,
                    exchange: 'huobi',
                    disTime: 5 * 60 * 1000,
                });
                tradeHandles[data.symbol].on('merge', function(symbol, data) {
            ...
            tradeHandles[data.symbol].merge(data.trade);
...
const write = throttle(function(insertData) {
    ...
    models.charts.depth.save(insertData);

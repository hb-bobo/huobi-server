 ...
 */
class StatisticalTrade extends Emitter{
    constructor({
        symbol,
        exchange = 'huobi',
        disTime = 5 * 60 * 1000
    ...
    merge(trade) {
        ...
        const _priceIndex = utils.getPriceIndex(symbol);
        // 先找缓存的数据是否存在
        ...
        if ((ts - preTime)  > this.disTime) {
            ...
            if (this._mergeData) {
                this._mergeData.buy = this._mergeData.buy.toFixed(2);
                this._mergeData.sell = this._mergeData.sell.toFixed(2);
...
module.exports = StatisticalTrade;
/**
 * 合并一个时间点的买卖交易量
